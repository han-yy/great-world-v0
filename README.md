...
网页只有一个主要输入框。参与者可以直接写“我去折页咖啡坐坐，问问林乔今天推荐什么”，也可以描述散步、观察、聊天、许愿或其他当下活动；不需要先选择动作类别。
...
每次提交都会返回至少一条属于当前参与者的可观察反馈。它可以是环境变化、他人的自然反应、行动遇到的阻力，或者“眼前没有出现新变化”；系统不会强迫 NPC 每次搭话，但也不会让一次表达无声消失。
...
观察台没有局部修改、发言、控制 Agent 或注入剧情的接口。唯一写操作是整世界重置：旧 epoch 被封存为只读，新 epoch 使用新 seed 启动；旧事件账本不会删除或改写。观察台查询不调用 DeepSeek，因此完整账本、world seed 和他人 cognition 不会被发送给模型供应商。
...
- `app/llm.py`：DeepSeek 意图/居民候选、上下文裁剪与失败关闭。
